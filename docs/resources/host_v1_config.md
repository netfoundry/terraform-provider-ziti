...
- `connect_timeout_seconds` (Number) Timeout in seconds when making outbound connections. NOTE: upstream host.v1 uses connectTimeout in preference when BOTH are set.
- `cost` (Number)
...
```shell
# host v1 config can be imported by specifying the identifier.
